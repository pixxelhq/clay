...
* Familiarity with [Types](IO.md) and the [Model Specification Format](spec.md).
...
Run the model using `clay run`:

```bash
clay run -e INPUT_JSON="$(cat sample_model_inputs.json)" ndvi:latest
```

The output will look something like:

```shell
Using configuration located at: /Users/xyz/work/example/models/NDVI/Ndvi/clay.yaml
WARNING - 2024-04-12 01:07:16,192 - runner.py:195 - job_model_runner - 'remote-prefix'
...
ERROR - 2024-04-12 01:07:17,200 - runner.py:403 - job_model_runner - Failed to fire callback
...
/Users/xyz/work/example/models/NDVI/Ndvi/model.py:34: RuntimeWarning: invalid value encountered in divide
  ndvi = (required_bands["B08"] - required_bands["B04"]) / (required_bands["B08"] + required_bands["B04"])
...
INFO - 2024-04-12 01:07:17,949 - runner.py:260 - job_model_runner - copying file from `result.tif` to `runs/wf123/job123/task123/outputs/result/result.tif`
WARNING - 2024-04-12 01:07:18,006 - core.py:372 - job_model_runner - `ORCHESTRATOR_URL` not set, and hence not firing callback
WARNING - 2024-04-12 01:07:18,006 - runner.py:426 - job_model_runner - failed to fire callback successfully
INFO - 2024-04-12 01:07:18,006 - runner.py:470 - job_model_runner - results: [Raster(Format='raster', Type='url', Name='result', Value='runs/wf123/job123/task123/outputs/result/result.tif', Default=None, IsArtifact=True, Properties=RasterProperties(Bands=None, Source=None, Collection='sentinel-s2-l2a-cogs', Dtype=None))]
```
...
Once your model is built and tested locally, you can:

- **Publish to Registry**: Use `clay publish` to push your model to the Clay registry
- **Deploy**: Deploy your model to your infrastructure or orchestrator
