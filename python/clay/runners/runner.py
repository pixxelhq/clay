...
from clay.utils import yaml_to_namespace


class RunnerConfig:
    # Environment variable keys
    _execution_id_env_key: Final[str] = "EXECUTION_ID"
    _input_json_env_key: Final[str] = "INPUT_JSON"
    ...
    _remote_output_path: str = "/tmp/outputs"
    ...
    def __init__(
        ...
        self._input_json_string: str = os.getenv(self._input_json_env_key, "[{}]")
        ...
        self._remote_output_path = os.getenv(self._remote_output_path_env_key, "/tmp/outputs")
    ...
    def _process_input_json(self) -> None:
        if self._input_json is None:
            if self._input_json_jq_filter:
                try:
                    self._input_json = json.loads(jq.compile(self._input_json_jq_filter).input(json.loads(self._input_json_string)).text())
                except Exception as e:
                    raise ValueError(f"Failed to process input JSON with jq filter '{self._input_json_jq_filter}': {e}")
                return

            self._input_json = json.loads(os.getenv(self._input_json_env_key, "[{}]"))
...
class JobRunner(BaseRunner):
    def __init__(
        ...
        self._callback_handler: CallbackInterface = HTTPCallback(
            callback_endpoint_prefix=self._params.get_callback_endpoint(),
    ...
    def _input_json_to_data_types(self, input_json: List[Dict[str, Any]]) -> None:
        ...
        for input_data in input_json:
            typed_input: datatypes.DataWrapperInterface = type_utils.TypeFromDict(input_data)
    ...
    def success(self) -> Any:
        ...
        self._callback_handler.send(
            ...
            inputs=self._params.get_input_json(),
    ...
    def start(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            self._inference_output = result._outputs_buffer  # type: ignore
            for output in self._inference_output:
            ...
            process_output_list(output_dict, self._params.get_remote_output_path())
